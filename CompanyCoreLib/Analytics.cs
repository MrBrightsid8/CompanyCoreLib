...
namespace CompanyCoreLib
{
	public class Analytics
	{
		...
		{
			List<Tuple<DateTime, int>> DateTimeWithCounterList = new List<Tuple<DateTime, int>>();
			...
			foreach( DateTime IterDate in dates )
			{
				if( IterDate.Year == PreviousYear )
				{
					...
					DateTime DateMonthStart = new DateTime(IterDate.Year, IterDate.Month, 1, 0, 0, 0);

					// ищем эту дату во временном списке
					int index = DateTimeWithCounterList.FindIndex(item => item.Item1 == DateMonthStart);

					// кортежи можно создавать по-разному
					if( index == -1 )
					{
						// такой даты нет - добавляем
